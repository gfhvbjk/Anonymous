...
class MeanFieldGaussianFeedForward(VIModule):
    ...
    def __init__(self,
        ...
        self.addLoss(lambda s: -self.out_features / 2 * np.log(2 * np.pi) - 0.5 * s.samples['wNoiseState'].pow(
            2).sum() - s.lweights_sigma.sum())
        ...
        if self.has_bias:
            ...
            self.addLoss(lambda s: -self.out_features / 2 * np.log(2 * np.pi) - 0.5 * s.samples['bNoiseState'].pow(
    ...
    def sampleTransform(self, stochastic=True):
        ...
        self.samples['weights'] = self.weights_mean + (
            torch.exp(self.lweights_sigma) * self.samples['wNoiseState'] if stochastic else 0)
...
class MeanFieldGaussian2DConvolution(VIModule):
    ...
    def __init__(self,
        ...
        self.weights_mean = Parameter((0. if initMeanZero else 1.) * (
                    torch.rand(out_channels, int(in_channels / groups), self.kernel_size[0],
                               self.kernel_size[1]) - 0.5))
        self.lweights_sigma = Parameter(torch.log(
            initPriorSigmaScale * wPriorSigma * torch.ones(out_channels, int(in_channels / groups), self.kernel_size[0],
                                                           self.kernel_size[1])))
        ...
        self.addLoss(lambda s: -self.out_channels / 2 * np.log(2 * np.pi) - 0.5 * s.samples['wNoiseState'].pow(
            2).sum() - s.lweights_sigma.sum())
