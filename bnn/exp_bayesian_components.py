...
class _BayesConvNd(Module):
    ...
    def reset_parameters(self, prior):
        ...
        if isinstance(self.prior_mu, torch.Tensor) and torch.numel(self.prior_mu) > 1:
            ...
            # nn.init.xavier_uniform_(self.weight_mu.data,gain=0.1)
            nn.init.normal_(self.weight_mu.data, mean=0.0, std=0.0001)
        # nn.init.normal_(self.weight_sigma.data, mean=0.0, std=0.0001)
        ...
        if self.bias:
            if isinstance(self.prior_bias_mu, torch.Tensor) and torch.numel(self.prior_bias_mu) > 1:
                ...
                nn.init.normal_(self.bias_mu.data, mean=0.0, std=0.0001)
...
class BayesConv2d(_BayesConvNd):
    ...
    def __init__(self, prior_mu, prior_sigma, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1,
                 groups=1, bias=False, padding_mode='zeros', bias_mu=0.0, prior=True):
    ...
    def forward(self, input):
        ...
        Overriden.
        """
        eps = Variable(torch.randn_like(self.weight_sigma))
        if torch.isnan(self.weight_sigma).all():
            raise ValueError("self.weight_sigmag have NaNs")
        weight = self.weight_mu + eps * torch.log(0.0003 + torch.exp(self.weight_sigma))
...
class BayesLinear(Module):
    ...
    def __init__(self, prior_mu, prior_sigma, in_features, out_features, bias_mu=False, bias=True, prior=True):
        ...
        self.register_buffer('weight_sigma_eps', None)
        # self.weight_sigma_eps = Variable(torch.randn_like(self.weight_sigma))
        ...
        if self.bias:
            ...
            self.register_buffer('bias_eps', None)
            # self.bias_eps.eps = Variable(torch.randn_like(self.weight_sigma))
    ...
    def reset_parameters(self, prior):
        ...
        if self.bias:
            ...
            nn.init.normal_(self.bias_sigma.data, mean=0.0, std=0.001)
    ...
    def forward(self, input):
        ...
        if self.bias:
            eps = torch.randn_like(self.bias_sigma)
            bias = self.bias_mu + eps * torch.log(0.0003 + torch.exp(self.bias_sigma))
...
class Convertor():
    ...
    def orig_to_bayes(model, prior):
        # prior_mu, prior_sigma, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1,
