...
def transform_model(input, from_inst, to_inst, args={}, attrs={}, inplace=True, _warn=False):
    if inplace:
        output = input
        if _warn:
    ...
    if isinstance(output, from_inst):
        ...
        for name, module in output.named_children():
            setattr(output, name,
                    transform_model(module, from_inst, to_inst, copy.deepcopy(args), copy.deepcopy(attrs), _warn=False))
