...
class _BayesConvNd(Module):
    ...
    def reset_parameters(self, prior):
        ...
        nn.init.xavier_uniform_(self.weight_sigma.data,gain=0.1)
        if self.bias:
...
class BayesConv2d(_BayesConvNd):
    ...
    def conv2d_forward(self, input, weight):
        if self.bias:
            ...
            bias = self.bias_mu + self.bias_sigma.eps * torch.log(0.01 + torch.exp((self.bias_sigma)))
    ...
    def forward(self, input):
        ...
        weight = self.weight_mu + eps * torch.log(0.01 + torch.exp(self.weight_sigma))
...
class BayesLinear(Module):
    ...
    def forward(self, input):
        ...
        self.weight_sigma.eps = Variable(torch.randn_like(self.weight_sigma))
        # std = torch.clamp(self.weight_sigma, min=2 ** -30)
        weight = self.weight_mu + self.weight_sigma.eps * torch.log(0.01 + torch.exp(self.weight_sigma))
        # else:
        #     self.weight_sigma.eps = Variable(torch.zeros_like(self.weight_sigma))
        #     weight = self.weight_mu
        ...
        if self.bias:
            ...
            bias = self.bias_mu + torch.log(0.01 + torch.exp(self.bias_sigma)) * self.bias_sigma.eps
